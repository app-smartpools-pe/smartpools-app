...
const FeaturedServices = () => {
  ...
  useEffect(() => {
    document.title = "Servicios destacados - smartpools";
    return () => {
      document.title = "smartpools"; // O el título por defecto
    };
  }, []);

  const obtenerServiciosDestacados = async () => {
    const res = await fetch("http://localhost:3000/api/featuredservices");
    const data = await res.json();
    setServiciosDestacados(data);
  };

  const eliminarServiciosDestacados = async (id) => {
    if (confirm("¿Eliminar este producto?")) {
      await fetch(`http://localhost:3000/api/featuredservices/${id}`, {
        method: "DELETE",
      });
  ...
  const filteredServiciosDestacados = serviciosDestacados.filter(
    (serv) =>
      serv.featuredservices.toLowerCase().includes(searchTerm.toLowerCase()) ||
      serv.descripcion.toLowerCase().includes(searchTerm.toLowerCase()) ||
      serv.price.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  return (
    <div className="globalManagerAllProducts-i-04-2025" role="main">
      <div
        data-i-gm-allfeaturedservices="inventory-4-2025"
              ...
              <div className="rs-allProducts-header-content">
                ...
                <div className="rs-allProducts-header-searchLink">
                  <div className="rs-allProducts-header-search">
                    <input
                      type="text"
                      placeholder="Buscar featured services en smartpools.pe"
                      value={searchTerm}
                    ...
                    <button
                      ...
                      onClick={filteredServiciosDestacados}
                    >
                  ...
                  <div className="rs-allProducts-header-link">
                    <Link to="/api/auth/manager/create/featured-services">
                  ...
                  <div className="tableAllProducs-content">
                    <thead className="tableAllProducts-header">
                      <tr>
                        ...
                        <th>
                          <div className="productListRow name">
                            Servicio Destacado
                          </div>
                        </th>
                        <th>
                    ...
                    <tbody className="tableAllProducts-body">
                      {filteredServiciosDestacados.map((serv) => (
                        <tr key={serv.id}>
                          <td>
                          ...
                          <td>
                            <div className="item-productListRow editCreate">
                              <div className="item-productEdit">
                                <Link
                                  to={`/manager/featured-services/editar/${serv.id}`}
                                >
                                  <svg
                                    viewBox="0 0 24 24"
                                    width={18}
                              ...
                              <div className="item-productDelete">
                                <button
                                  onClick={() =>
                                    eliminarServiciosDestacados(serv.id)
                                  }
                                >
                                  <svg
                                    viewBox="0 0 24 24"
